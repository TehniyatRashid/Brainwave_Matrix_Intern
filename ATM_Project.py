...
import pyrebase
from firebase_connect import database
...
class ATM:
    ...
    def Regestration_track(self):
        ...
        if existing_account.val() is None :
            database.child("Registered Users").child(self.data["account_no"]).set(self.data)
    ...
    def services_window(self):
        ...
        user_pin = database.child("Registered Users").child(acc_no).child("pin_code").get()
    ...
    def click_deposit(self):
        ...
        database.child("Registered Users").child(self.acc_no_entry.get()).update({"balance": current_bal})
    ...
    def change_pin(self):
        ...
        if old_pin_fb.val() == old_pin:
            database.child("Registered Users").child(self.acc_no_entry.get()).update({"pin_code": new_pin})
